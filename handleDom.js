...
export function createInputs(colorsList, callback) {
  ...
  colorsList.forEach((colorObj, index) => {
    const input = ml('input', {type:'color', value: colorObj.hex, oninput: (e) => {
      colorObj.newColor = e.target.value
      callback(colorObj)
    }})
    inputsList.push(input)
    const colorContainerEl = ml('label', {}, [
      ml('small', {}, colorObj.origin),
      ml('span', {style: 'background:'+colorObj.origin}),
      ml('div', {}, [
